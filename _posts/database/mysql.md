...
| INT或INTEGER/int或integer | 4 bytes                                   | (-2 147 483 648，2 147 483 647)                              | (0，4 294 967 295)                                           | 大整数值        |
| BIGINT/bigint             | 8 bytes                                   | (-9,223,372,036,854,775,808，9 223 372 036 854 775 807)      | (0，18 446 744 073 709 551 615)                              | 极大整数值      |
| FLOAT/float               | 4 bytes                                   | (-3.402 823 466 E+38，-1.175 494 351 E-38)，0，(1.175 494 351 E-38，3.402 823 466 351 E+38) | 0，(1.175 494 351 E-38，3.402 823 466 E+38)                  | 单精度 浮点数值 |
| DOUBLE/double             | 8 bytes                                   | (-1.797 693 134 862 315 7 E+308，-2.225 073 858 507 201 4 E-308)，0，(2.225 073 858 507 201 4 E-308，1.797 693 134 862 315 7 E+308) | 0，(2.225 073 858 507 201 4 E-308，1.797 693 134 862 315 7 E+308) | 双精度 浮点数值 |
| DECIMAL/decimal           | 对DECIMAL(M,D) ，如果M>D，为M+2,否则为D+2 | 依赖于M和D的值                                               | 依赖于M和D的值                                               | 小数值          |
...
| DATETIME/datetime   | 8             | 1000-01-01 00:00:00/9999-12-31 23:59:59                      | YYYY-MM-DD HH:MM:SS | 混合日期和时间值         |
| TIMESTAMP/timestamp | 4             | 1970-01-01 00:00:00/2038结束时间是第 **2147483647** 秒，北京时间 **2038-1-19 11:14:07**，格林尼治时间 2038年1月19日 凌晨 03:14:07 | YYYYMMDD HHMMSS     | 混合日期和时间值，时间戳 |
...
# unsigned  
# auto_increment 自增,一般用于主键, 数值会自动加一
# not null 不可以为空
# engine 设置存储引擎
# default charset = utf8 设置默认编码
```
...
```mysql
insert into table_name ( field1, field2 ... ) values ( values1, values ... )
```
...
>   - = | <> | != | > | < | >= | <=
>
>   - binary : 区分大小写 `where binary column_name = 123`
>
>   - like : 类似等号 , 可以和%%一起使用`where field1 like %123%`
>
>     - ```mysql
>       '%a'     //以a结尾的数据
>       'a%'     //以a开头的数据
...
# 将 C++ 替换为 Python
UPDATE db_tbl SET db_title = REPLACE(db_title, 'C++', 'Python') where 
db_id = 3;
```
...
> - inner join ( 内连接, 或等值连接 ): 获取两个表中字段匹配关系记录 ( 可省略inner )
...
| ------------------------------------------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ |
| <img src="https://images.gitee.com/uploads/images/2020/0830/180247_af990851_7984151.png" style="zoom: 50%;" /> | <img src="https://images.gitee.com/uploads/images/2020/0830/180208_31de2c10_7984151.png" style="zoom:50%;" /> | <img src="https://images.gitee.com/uploads/images/2020/0830/180132_3af0b806_7984151.png" style="zoom:50%;" /> |

### NULL 值处理

> - is null 
> - is not null
> - <=> :比较两个值相等 或 都为 Null 时, 才会返回true
